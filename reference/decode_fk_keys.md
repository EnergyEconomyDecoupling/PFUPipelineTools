...
a vector of fk keys (integers, `v_key`) into a vector fk values
(strings) according to the database at `conn`.
...
- v_key:
  ...
  A vector of fk keys (typically integers) to be converted to fk values
  (typically strings).
...
`v_key` decoded according to `fk_parent_tables`.
