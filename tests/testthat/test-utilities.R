...
test_that("inboard_filter_copy() works as expected", {
  ...
  source_table <- data.frame(Country = c(1, 1, 1,
                                         2, 2, 2,
                                         3, 3, 3),
  ...
  DM <- dm::as_dm(list(source = source_table[0, ],
    ...
    dm::dm_add_fk(table = source, columns = Country,
                  ref_table = Country, ref_columns = CountryID) |>
    dm::dm_add_fk(table = source, columns = Year,
                  ref_table = Year, ref_columns = YearID) |>
    dm::dm_add_fk(table = dest, columns = Country,
                  ref_table = Country, ref_columns = CountryID) |>
    dm::dm_add_fk(table = dest, columns = Year,
                  ref_table = Year, ref_columns = YearID)


  dm::copy_dm_to(dest = conn, dm = DM, set_key_constraints = TRUE)

  DBI::dbWriteTable(conn, "Country", country_table, overwrite = TRUE)
  DBI::dbWriteTable(conn, "Year", year_table, overwrite = TRUE)
